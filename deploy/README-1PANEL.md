...
| 文件 | 说明 | 必需 |
|------|------|------|
| `qywx-notifier-1.0.0.tar.gz` | 打包好的 Docker 镜像 | ✅ |
...
> 已由开发者完成，产物为 `deploy/qywx-notifier-1.0.0.tar.gz`。
> 如需自行重建，执行：
...
产物：`deploy/qywx-notifier-1.0.0.tar.gz`。

---

## 四、第二步：上传到 1Panel 服务器

把 `deploy/` 目录下的这些文件，上传到服务器**同一个目录**（建议放 `/opt/qywx-notifier/`）：

- `qywx-notifier-1.0.0.tar.gz`
- `docker-compose.yml`
- `.env.example`
...
2. 点击 **「导入镜像」**
3. 选择上传的 `qywx-notifier-1.0.0.tar.gz`（或先解压成 `.tar`）
4. 确认导入，列表中出现 `qywx-notifier:1.0.0` 即成功

### 方式 B：命令行导入

```bash
cd /opt/qywx-notifier
gunzip -c qywx-notifier-1.0.0.tar.gz | docker load
# 验证
docker images | grep qywx-notifier
```
...
3. 填写：
   - **名称**：`wechat-notifier`
   - **编排目录**：填上传文件的目录，如 `/opt/qywx-notifier`
...
docker compose ps
# 2. 健康检查为 healthy
docker inspect --format='{{.State.Health.Status}}' wechat-notifier
# 3. 端口可达
...
| 进入容器排查 | `docker exec -it wechat-notifier sh` |

### 数据备份与恢复

数据存在 Docker 命名卷 `wechat-notifier-data`：

```bash
# 备份
docker run --rm -v wechat-notifier-data:/data -v "$PWD":/backup alpine \
...
**Q2：1Panel 界面看不到这个镜像？**
→ 镜像是 `docker load` 导入的本地镜像，在 **「容器→镜像」** 列表能看到；编排里用 `image: qywx-notifier:1.0.0` 引用即可，无需推到仓库。
