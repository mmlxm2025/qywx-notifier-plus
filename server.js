...
// 管理/API 响应禁止 ETag/304，避免认证状态与配置元数据落入浏览器缓存。
// express.static 仍会为静态文件维护自己的协商缓存，不受此设置影响。
app.disable('etag');
app.use(securityHeaders);

// CACHE-001：所有 API（含登录、通知、回调）统一禁止浏览器与中间代理缓存。
...
// QA-001：仅隔离测试实例开放的认证后探针。生产未设置 TEST_INSTANCE_ID 时保持 404，
// live 套件必须在任何数据写入前比对精确实例 ID，防止误打生产环境。
app.get('/api/test-environment', (req, res) => {
    if (!config.raw.testInstanceId) {
        return res.status(404).json({ error: '未找到资源', code: 'NOT_FOUND' });
    }
    const sessionId = auth.parseSessionFromCookie(req.headers.cookie);
    if (!auth.verifySession(sessionId)) {
        return res.status(401).json({ error: '未登录或登录已过期', code: 'AUTH_REQUIRED' });
    }
    return res.json({ test_env: true, instance_id: config.raw.testInstanceId });
});
...
app.post('/api/login', async (req, res) => {
    ...
    return res.status(401).json({ error: result.error, code: 'AUTH_INVALID' });
});
...
app.get('/health/version', (_req, res) => {
    res.json({ version: config.raw.appVersion, revision: config.raw.appRevision });
});
