...
test('runtime server: API no-store、稳定登录错误码、测试实例探针与版本信息', async t => {
    ...
    // P3：会话管理 401 必须带稳定 AUTH_REQUIRED（与 requireAuth 契约一致）。
    res = await fetch(`${base}/api/sessions`);
    assert.equal(res.status, 401);
    assert.equal((await res.json()).code, 'AUTH_REQUIRED');

    res = await fetch(`${base}/api/sessions/abc`, { method: 'DELETE' });
    assert.equal(res.status, 401);
    assert.equal((await res.json()).code, 'AUTH_REQUIRED');

    res = await fetch(`${base}/api/sessions`, { method: 'DELETE' });
    assert.equal(res.status, 401);
    assert.equal((await res.json()).code, 'AUTH_REQUIRED');

    // P3：全局 404 补稳定 code，前端禁止依赖中文文案分支。
    res = await fetch(`${base}/api/this-route-does-not-exist-xyz`);
