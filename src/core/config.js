...
function readRawConfig() {
    return {
        ...
        // 仅隔离测试实例设置。生产环境留空时测试环境探针保持 404。
        testInstanceId: process.env.TEST_INSTANCE_ID || '',
        // 由 Docker 构建参数注入，用于运行制品追溯；本地启动使用 package 版本兜底。
        appRevision: process.env.APP_REVISION || 'development',
...
function validateRuntime() {
    ...
    if (raw.testInstanceId && String(raw.testInstanceId).trim().length < 16) {
        errors.push('TEST_INSTANCE_ID 至少需要 16 个字符；生产环境不应设置该变量。');
    }
